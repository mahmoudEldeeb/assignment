...
/**
 * this my second page at this page  i can take image and save my visit details like images ,location temoreture ,...
 * @author Areej
 */
...
/// get date and time
                ...
                /// check if has camera or not and ask permission to pickup images
                if(hasCamera()) {
...
///
    ...
    public void onActivityResult(int requestCode, int resultCode, Intent data) {
        ...
        // check if i insert visit in database or not yet visitId is id of visit inserted in room
        if(visitId==0) {
            // here ou must put longitude and latitude
            if(myLocation!=null) {

                // here create visit object and insert it into room
...
Log.v("qqqq",title + ""+"   "+description+"   "+time+"   "+ myLocation.getLongitude()+"   "+
        myLocation.getLatitude()+"   "+ temperature.getLastReportTime()
        +"   "+ barometer.getLastReportTime());
                insertVisitsViewModel.insertVisit(visit).
                        observe(Main2Activity.this, new Observer<Long>() {
                    @Override
        ...
        // i check here if image come from gallard or from camera
        if (requestCode == GALLERY) {
            if (data != null) {
                Uri contentURI = data.getData();
                try {
                    Bitmap bitmap = MediaStore.Images.Media.getBitmap(this.getContentResolver(), contentURI);
                    //call saveimage to convert it to bytearrya and put it on list of images
                     saveImage(bitmap);
...
/// here i create object from VisitImage and add to list
        ...
        VisitImages visitImage=new VisitImages(visitId,byteArray);
